 ...
 switch(vArg.charAt(0))
            {
                case '#':
                    var aEle=document.getElementById(vArg.substring(1));
                    this.elements.push(aEle);
                break;
                case '.':
                    //.box //.box box2 box3
                    var aELe1=getByClass(document,vArg.substring(1));
                    this.elements=aELe1;
                break;
                default:
                    if(vArg.indexOf('.')!=-1)
                    ...
                    //div.box
                        var arr=vArg.split('.');           
                        var aSelect0=document.getElementsByTagName(arr[0]);
                        var oSelect1='\\b'+arr[1]+'\\b';
                        for(i=0;i<aSelect0.length;i++)
                        {              
                            if(aSelect0[i].className.search(oSelect1)!=-1)
                            {
                                this.elements.push(aSelect0[i]);
                            }
                        }                
                    }
                    else
                    {
                        //div
                        var aELe2=document.getElementsByTagName(vArg);
                        this.elements=aELe2;                        
                    }           
                break;
            }     